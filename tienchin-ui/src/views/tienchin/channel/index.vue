    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
            ...
            icon="Upload"
            @click="handleImport"
            v-hasPermi="['tienchin:channel:import']">导入
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
      ...
      <el-table-column label="渠道状态" :show-overflow-tooltip="true" width="100">
        <template #default="scope">
          <template v-for="(type, index) in channel_status">
            <el-tag :key="index" v-if="type.value == scope.row.status" :type="type.elTagType">{{type.label}}</el-tag>
          </template>
        </template>
      </el-table-column>
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除角色编号为"' + channelIds + '"的数据项?').then(function () {
...
/** 多选框选中数据，点击表格中的第一列选择框，这个方法就会被触发 */
...
/** 修改渠道，无论是点击最上面的修改按钮，还是点击表格中的修改按钮，都是调用了这个方法，不同的是，如果是点击表格中的修改按钮，调用此方法则传递了参数 */
function handleUpdate(row) {
  reset();
  //这个渠道 ID 有两种来源：如果是点击表格中的修改按钮，则通过 row.channelId 来获取要修改的渠道 ID
  //如果是点击表格上方的修改按钮，则是通过 ids.value 来获取要修改的渠道 ID
