...
const usePermissionStore = defineStore(
  'permission',
  {
    state: () => ({
      /**
       * 这个保存了本地的公共路由和服务端返回的动态（路由）菜单，将来主要是在两个地方使用：
       * 1、首页搜索时使用
       * 2、tagview 中使用
       *
       * 从 routes 的使用场景中，可以看到，routes 变量实际上并不涉及到页面渲染，单纯的只是处理页面的逻辑
       *
       * 这个 routes 有一个区别于下面三个变量的特点，这个 routes 中的 children 铺平了：就是把所有的 3-n 菜单，全部变为 2 级菜单，这个铺平操作一共做了两件事：
       *
       * 1、所有的 3-n 级菜单，全部变为 2级菜单
      ...
      addRoutes: [],
      /**
       * 这个相当于是动态路由的一个备份，也可以理解为是完整的 sidebarRouters 的备份，无论什么情况，defaultRoutes 都表示 1-n级菜单
       */
      defaultRoutes: [],
      /**
    ...
    actions: {
      ...
      generateRoutes(roles) {
        return new Promise(resolve => {
          // 向后端请求路由数据
          getRouters().then(res => {
            //这个类似于 Java 中的深拷贝，如果直接将服务端返回的 JSON 变量赋值给三个不同的变量，那么三个变量会指向同一块内存地址，这样就导致了一个变量发送变化，另外两个变量会跟着变。
            //将JSON 转为字符串，再将字符串转为 JSON，这样三个变量就使用三块不同的内存地址，将来在具体应用的过程中就互不影响
            //Object。assign 也可以实现
            const sdata = JSON.parse(JSON.stringify(res.data))
            const rdata = JSON.parse(JSON.stringify(res.data))
            const defaultData = JSON.parse(JSON.stringify(res.data))
            //这个变量将服务端返回的动态 JSON 中的 component 字符串变为了对象
            const sidebarRoutes = filterAsyncRouter(sdata)
            //这是一个重写后的 routes，这个重写后的routes 主要是将 3-n 级菜单统统变为 2 级菜单
            const rewriteRoutes = filterAsyncRouter(rdata, false, true)
            //这个变量将服务端返回的动态 JSON 中的 component 字符串变为了对象
            const defaultRoutes = filterAsyncRouter(defaultData)
            //动态路由，根据用户权限查询出本地定义的可用的动态路由，这些动态路由都不涉及到菜单渲染，所以查询到之后，直接遍历，放到 router 对象即可
            const asyncRoutes = filterDynamicRoutes(dynamicRoutes)
            asyncRoutes.forEach(route => { router.addRoute(route) })
            //这个实际上前端公共路由和服务端返回的动态路由，赋值给 routes
            this.setRoutes(rewriteRoutes)
            //左侧菜单栏渲染，这个也是公共路由 + sidebarRoutes
            this.setSidebarRouters(constantRoutes.concat(sidebarRoutes))
            //这个中永远保存的是最完整的菜单项数据，所以也是 constantRoutes + sidebarRoutes，只不过是在 set 方法中加的
...
/**
 * 根据当前用户的权限，过滤出本地定义的动态路由中，有哪些是可用的
 * @param routes
 * @returns {*[]}
 */
// 动态路由遍历，验证是否具备权限
